...
#define TMX_DIR "/Users/temporaryadmin/.config/tmuxinator"
...
static int get_callback(void *not_used, int argc, char **argv,
  ...
  if (T.is_favourite == 0) {
    if (strcmp(argv[1], "1") == 0) {
      fprintf(T.fzf_file, "\x1b[33;49;4m* %s *\x1b[0m\n", argv[0]);
    } else {
      fprintf(T.fzf_file, "%s\n", argv[0]);
    }
  } else {
    if (strcmp(argv[1], "1") == 0) {
...
static int ptree(char *file_path) {
  FTS *ftsp;
  FTSENT *p, *chp;
...
void open_fzf() {
  char *command = "fzf-tmux -p 50% --multi --reverse --ansi"
                  " --bind "
                  "'ctrl-f:reload( "
                  "./mx-fzf favourite {})' | tr -d '\r'  | xargs -L 1 "
                  "tmuxinator start ";
  FILE *fp;
  fp = popen(command, "w");
  T.fzf_file = fp;
